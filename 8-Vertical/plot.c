...
static void cell_draw_refpos(int m, int n, int w, int h)
{
  for (int t = 0; t < TRACE_COUNT; t++) {
    ...
    int y = 8*GRIDY - floatToInt(get_trace_refpos(t) * GRIDY) - n * CELLHEIGHT - Y_REFERENCE_OFFSET;
...
void draw_frequencies(void)
{
  ...
  setBackgroundColor(DEFAULT_BG_COLOR);
#if !defined(ANTENNA_ANALYZER)
  ili9341_fill(0, 233, 320, 8, DEFAULT_BG_COLOR);
  ili9341_drawstring(buf1, OFFSETX, 233);
  ili9341_drawstring(buf2, 205, 233);
#else
